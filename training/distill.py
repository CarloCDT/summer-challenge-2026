...
def _play_and_label(args):
    """One game. Returns per-turn states + teacher logits, and per-decision masks.
    ...
    while not sim.is_game_over():
        ...
        if key != cur_state_key:
            with torch.no_grad():
                tl, _ = teacher(torch.from_numpy(state).unsqueeze(0))
    ...
    return (np.stack(states), np.stack(tlogits), np.stack(masks),
...
def collect(pool, teacher, teacher_kwargs, student, student_kwargs, seeds,
    ...
    states, tlogits, masks, index = [], [], [], []
    offset = 0
    for st, tl, mk, ix in results:
        states.append(st); tlogits.append(tl); masks.append(mk); index.append(ix + offset)
        offset += len(st)
    return (np.concatenate(states), np.concatenate(tlogits),
...
def distill(
    ...
    try:
        for it in range(num_iterations):
            ...
            states, tlogits, masks, index = collect(
            ...
            print(f"[iter {it}] kl={kl:.4f} agree={agree:.3f} beta={beta:.2f} "
